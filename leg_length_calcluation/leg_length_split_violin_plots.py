...
for trial_name, session in zip(trial_names, list_of_sessions):
    ...
    trial_labels.extend([trial_name] * (len(freemocap_leg_lengths) + len(qualisys_leg_lengths)))
...
sns.violinplot(x='Trial', y='Leg Length', hue='System', data=data, split=True, inner='quart', palette='pastel')
plt.title('Comparison of Leg Lengths between FreeMoCap and Qualisys')
