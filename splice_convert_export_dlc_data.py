...
class DLCDataWorker:
    ...
    def __init__(self, path_to_recording_folder, output_folder_name, raw_data_file_name, path_to_calibration_toml, task_list=[]):
        ...
        self.available_tasks = {
            TASK_COMPILE_CSVS: self.compile_csvs,
            TASK_RECONSTRUCT_DLC_DATA: self.reconstruct_data,
            TASK_SPLICE_3D_DATA: self.splice_data,
    ...
    def reconstruct_data(self):
        dlc_2d_data = self.tasks[TASK_COMPILE_CSVS]['result']
        return reconstruct_dlc_data(dlc_2d_data, self.path_to_calibration_toml)

    def splice_data(self):
        dlc_3d_data = self.tasks[TASK_RECONSTRUCT_DLC_DATA]['result']
        return splice_freemocap_and_dlc_leg_data(dlc_3d_data, freemocap_3d_data=self.freemocap_data, show_graph=False)
